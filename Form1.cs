...
namespace Lab1_compile
{
    ...
    public partial class Form1 : Form
    {
                ...
                while (stroke.Next().Char != '\0')
                {
                    ...
                    foreach (var error in errors)
                    {
                        ...
                        if (error.IncorrStr != null)
                        {
                            ...
                            editor.Select(error.Idx, error.IncorrStr.Length);  // Выделяем весь неправильный фрагмент
                        }
                        else
                        {
                            editor.Select(error.Idx, 1);  // Если фрагмент не указан, выделяем один символ
                        }
        ...
        private void постановкаToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                // Сохраняем встроенный HTML в файл
                File.WriteAllText(tempPath, Properties.Resources.task);
                // Открываем в браузере
                Process.Start(new ProcessStartInfo(tempPath) { UseShellExecute = true });
            }
            catch (Exception ex)
        ...
        private void методАнализаToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string html = Properties.Resources.analysis;
